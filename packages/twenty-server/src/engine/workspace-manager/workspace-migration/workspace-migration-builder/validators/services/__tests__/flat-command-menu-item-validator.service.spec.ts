...
const OBJECT_UNIVERSAL_IDENTIFIER = 'd1d2d3d4-d5d6-4000-8000-000000000001';
const OTHER_OBJECT_UNIVERSAL_IDENTIFIER =
  'd1d2d3d4-d5d6-4000-8000-000000000002';

const buildFlatObjectMetadataMaps = (
  commandMenuItemUniversalIdentifiersByObjectUniversalIdentifier: Record<
    string,
    string[]
  >,
) => ({
  byUniversalIdentifier: Object.fromEntries(
    Object.entries(
      commandMenuItemUniversalIdentifiersByObjectUniversalIdentifier,
    ).map(
      ([objectUniversalIdentifier, commandMenuItemUniversalIdentifiers]) => [
...
describe('FlatCommandMenuItemValidatorService', () => {
  ...
  describe('validateFlatCommandMenuItemCreation', () => {
    ...
    it('rejects a second navigation command for the same object', () => {
      const result = validator.validateFlatCommandMenuItemCreation(
        buildValidationArgs({
          ...
          commandMenuItemUniversalIdentifiersByObjectUniversalIdentifier: {
            [OBJECT_UNIVERSAL_IDENTIFIER]: [
              'other-command-menu-item-identifier',
            ],
          },
        }),
      );
    ...
    it('does not catch a path-based NAVIGATION command carrying an empty objectMetadataItemId', () => {
      const result = validator.validateFlatCommandMenuItemCreation(
        buildValidationArgs({
          flatEntityToValidate: buildFlatCommandMenuItem({
            universalIdentifier: 'go-to-settings-identifier',
            payload: { path: '/settings/profile', objectMetadataItemId: '' },
            navigationTargetObjectMetadataUniversalIdentifier: null,
            isSystemSideEffect: false,
          }),
        }),
    ...
    it('rejects an update retargeting a row onto an object that already has a navigation command', () => {
      const result = validator.validateFlatCommandMenuItemUpdate(
        buildUpdateArgs({
          ...
          commandMenuItemUniversalIdentifiersByObjectUniversalIdentifier: {
            [OBJECT_UNIVERSAL_IDENTIFIER]: ['existing-navigation-identifier'],
          },
          existingFlatCommandMenuItems: [
            buildFlatCommandMenuItem({
              universalIdentifier: 'retargeted-identifier',
